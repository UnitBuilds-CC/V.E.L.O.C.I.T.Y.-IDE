...
impl DualPathEngine {
    ...
    pub fn open(
        ...
        let head_path = if head_path.exists() { Some(head_path) } else { None };
    ...
    fn run_path2(&mut self, prompt: &str, max_tokens: usize) -> Result<EngineOutput> {
        ...
        eprintln!("[bridge] Output vocabulary: {} opcodes (zero-hallucination mode)",
        ...
        let path1 = self.path1.as_mut().unwrap();
        let prompt_tokens = self.tokenizer.encode(prompt, true);
        let condition = path1.transformer.get_conditioning_hidden_state(&prompt_tokens);

        let mut opcodes = Vec::new();
        let t_start     = Instant::now();

        let result = self.path2.generate(
            Some(&condition),
            max_tokens,
            |op| {
                print!(" {}", op.name());
        ...
        if result.valid {
            let status = if result.force_terminated { "VALID but TRUNCATED" } else { "VALID (complete)" };
            ...
            if let Some(ref sc) = result.scope {
                ...
                let comment = if sc.passed { "" } else { " — not stored (prompt-program misalignment)" };
                eprintln!("[bridge] Scope      : {}  sim={:.2} (θ={:.2}){}", status, sc.similarity, sc.threshold, comment);
            }
        ...
        eprintln!("[bridge] SiteMap Cache stats: {} hits, {} misses",
...
pub fn run_dual_path(
    ...
    match result {
        EngineOutput::Text { n_tokens, elapsed_ms, .. } => {
        ...
        EngineOutput::Nda { n_opcodes, elapsed_ms, valid, root_hash, force_terminated, .. } => {
            let elapsed_s = elapsed_ms as f64 / 1000.0;
