...
pub fn run_providers(args: ProvidersArgs, json: bool) -> Result<()> {
    ...
    match args.action.as_str() {
        "list" => {
            ...
            println!("  {}", "-".repeat(60));
            for c in &creds {
                let masked = if c.api_key.len() > 12 { format!("{}...{}", &c.api_key[..8], &c.api_key[c.api_key.len()-4..]) } else { "****".into() };
                println!("  {:<16} {:<20} {}", c.provider, masked, c.base_url.as_deref().unwrap_or("(default)"));
            }
            println!();
        }
        "add" => {
            let provider = args.provider.as_deref().ok_or_else(|| anyhow::anyhow!("--provider is required (e.g. --provider openai)"))?;
            let api_key = args.api_key.as_deref().ok_or_else(|| anyhow::anyhow!("--api-key is required"))?;
            ...
            creds.push(ProviderCredential { provider: provider.to_lowercase(), api_key: api_key.to_string(), base_url: args.base_url.clone(), model: None });
            save_credentials(&creds)?;
            println!("\nProvider API key saved:");
            println!("  Provider:  {}", provider);
            println!("  Key:       {}...{}", &api_key[..4], &api_key[api_key.len().saturating_sub(4)..]);
            if let Some(ref url) = args.base_url { println!("  Base URL:  {}", url); }
            println!("  Stored in: ~/.velocity/providers.toml\n");
            println!("Run `velocity-ide providers refresh` to query usage.\n");
        }
        "remove" => {
            let provider = args.provider.as_deref().ok_or_else(|| anyhow::anyhow!("--provider is required"))?;
            let mut creds = load_credentials()?;
            let before = creds.len();
            creds.retain(|c| c.provider.to_lowercase() != provider.to_lowercase());
            if creds.len() == before { println!("No provider '{}' found in configuration.", provider); }
            else { save_credentials(&creds)?; println!("Provider '{}' removed.", provider); }
            println!();
        }
        "refresh" => {
            let creds = load_credentials()?;
            if creds.is_empty() {
                if json { println!("{{}}"); return Ok(()); }
                println!("\nNo provider API keys configured.\n");
                return Ok(());
            }
            let snapshot = velocity_ide::provider_usage::query_all_providers(&creds);
            if json { println!("{}", serde_json::to_string_pretty(&snapshot)?); return Ok(()); }
            println!("\nQuerying {} provider(s)...\n", creds.len());
            println!("  {:<16} {:<8} {:>12} {:>10}  Status", "Provider", "Valid", "Tokens", "Cost");
            println!("  {}", "-".repeat(75));
            for p in &snapshot.providers {
                println!("  {:<16} {:<8} {:>12} {:>10}  {}", p.display_name, if p.key_valid { "yes" } else { "NO" },
                    velocity_ide::velocity_client::fmt_number(p.tokens_used),
                    velocity_ide::velocity_client::fmt_currency(p.cost_usd), p.status);
            }
        ...
        other => anyhow::bail!("Unknown action '{}'. Use: list, add, remove, refresh", other),
    }
    Ok(())
}
...
mod tests {
    use super::*;
    #[test] fn providers_args_optional_fields() {
        let a = ProvidersArgs { action: "list".into(), provider: None, api_key: None, base_url: None };
        assert!(a.provider.is_none()); assert!(a.api_key.is_none()); assert!(a.base_url.is_none());
    }
    #[test] fn providers_args_all_fields() {
        let a = ProvidersArgs { action: "add".into(), provider: Some("openai".into()), api_key: Some("sk-test".into()), base_url: Some("https://proxy.example.com".into()) };
