...
pub fn run_transparency(json: bool) -> Result<()> {
    ...
    if json { println!("{}", serde_json::to_string_pretty(&t)?); return Ok(()); }
    ...
    println!("  Cost Flow:");
    println!("    Input tokens:  {} ({:.1}%)", fmt_number(t.cost_flow.input_tokens), if total_tok > 0 { t.cost_flow.input_tokens as f64 / total_tok as f64 * 100.0 } else { 0.0 });
    println!("    Output tokens: {} ({:.1}%)", fmt_number(t.cost_flow.output_tokens), if total_tok > 0 { t.cost_flow.output_tokens as f64 / total_tok as f64 * 100.0 } else { 0.0 });
    ...
    if !t.recent_routing_decisions.is_empty() {
        ...
        for d in t.recent_routing_decisions.iter().take(20) {
            ...
            let short = if r.len() > 40 { format!("{}...", &r[..37]) } else { r.to_string() };
    ...
    if !t.model_selection_stats.is_empty() {
        ...
        println!("  {}", "-".repeat(70));
        for m in &t.model_selection_stats { println!("  {:<24} {:>8} {:>12} {:>10} {:>10}", m.model_id, m.total_requests, fmt_number(m.total_tokens), fmt_currency(m.total_cost_usd), m.avg_duration_ms); }
        println!();
    }
    if !t.domain_distribution.is_empty() {
        ...
        println!("  {}", "-".repeat(60));
        for d in &t.domain_distribution { println!("  {:<24} {:>8} {:>12} {:>10}", d.domain, d.requests, fmt_number(d.tokens), fmt_currency(d.cost_usd)); }
        println!();
    }
    if !t.available_models.is_empty() {
        ...
        println!("  {}", "-".repeat(76));
        for m in &t.available_models { println!("  {:<24} {:<14} {:<10} {:>12.2} {:>12.2}", m.id, m.provider, m.tier, m.cost_input_per_mtok, m.cost_output_per_mtok); }
        println!();
    }
    Ok(())
}
