...
pub fn lex(input: &str) -> Result<Vec<Token>, String> {
    ...
    while i < chars.len() {
        ...
        match c {
            ...
            '+' => {
                i += 1;
                tokens.push(if i < chars.len() && chars[i] == '+' {
                    i += 1;
                    Token::PlusPlus
            ...
            c if c.is_ascii_digit() => {
                ...
                if c == '0' && i + 1 < chars.len() && (chars[i + 1] == 'x' || chars[i + 1] == 'X') {
                    ...
                    tokens.push(Token::Number(
                        i64::from_str_radix(&num, 16).unwrap_or(0) as f64
                    ));
                } else {
...
mod tests {
    ...
    fn shift_operators() {
        ...
        assert_eq!(
            tokens,
            vec![Token::LtLt, Token::GtGt, Token::GtGtGt, Token::Eof]
        );
    }
    ...
    fn template_literal() {
        ...
        assert_eq!(
            tokens,
            vec![Token::Template("hello world".into()), Token::Eof]
        );
    }
    ...
    fn decimal_number() {
        ...
        assert_eq!(
            tokens,
            vec![Token::Number(42.0), Token::Number(2.72), Token::Eof]
        );
    }
    ...
    fn hex_number() {
        ...
        assert_eq!(
            tokens,
            vec![Token::Number(255.0), Token::Number(16.0), Token::Eof]
        );
    }
    ...
    fn scientific_notation() {
        ...
        assert_eq!(
            tokens,
            vec![Token::Number(1000.0), Token::Number(0.025), Token::Eof]
        );
    }
    ...
    fn keywords() {
        let tokens =
            lex("var let const function return if else while for do break continue").unwrap();
        assert_eq!(
            tokens,
            vec![
                Token::Var,
