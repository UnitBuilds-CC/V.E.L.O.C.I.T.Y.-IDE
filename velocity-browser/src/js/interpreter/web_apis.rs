...
pub(super) fn call_abort_controller_method(
...
pub(super) fn call_abort_signal_method(
    map: &HashMap<String, JsValue>,
    method: &str,
    _args: &[JsValue],
) -> EvalResult {
...
pub(super) fn call_text_encoder_method(
    _map: &HashMap<String, JsValue>,
    method: &str,
    args: &[JsValue],
) -> EvalResult {
    Ok(match method {
...
pub(super) fn call_text_decoder_method(
    _map: &HashMap<String, JsValue>,
    method: &str,
    args: &[JsValue],
) -> EvalResult {
...
pub(super) fn call_response_method(
    ...
    Ok(match method {
        ...
        "statusText" => map
            .get("statusText")
            .cloned()
            .unwrap_or(JsValue::String("OK".to_string())),
        "headers" => map
            .get("headers")
            .cloned()
...
pub(super) fn call_blob_method(
    ...
    Ok(match method {
        ...
        "arrayBuffer" => match map.get("__data__") {
            Some(JsValue::String(s)) => JsValue::Array(
                s.as_bytes()
                    .iter()
                    .map(|b| JsValue::Number(*b as f64))
                    .collect(),
...
pub(super) fn call_typed_array_method(
    map: &HashMap<String, JsValue>,
    method: &str,
    _args: &[JsValue],
) -> EvalResult {
    Ok(match method {
        ...
        "set" => {
            let mut updated = map.clone();
            if let (Some(JsValue::Array(src)), Some(JsValue::Array(data))) =
                (_args.first(), map.get("__data__"))
            {
        ...
        "slice" => {
            ...
            new_map.insert(
                ...
                map.get("__type__")
                    .cloned()
                    .unwrap_or(JsValue::String("TypedArray".to_string())),
            );
        ...
        "fill" => {
            ...
            if let Some(JsValue::Array(data)) = map.get("__data__") {
                ...
                let end_idx = if end == 0 {
                    ...
                    end.min(new_data.len())
                };
                for i in start..end_idx {
                    new_data[i] = val.clone();
                }
                updated.insert("__data__".to_string(), JsValue::Array(new_data));
            }
            JsValue::Object(updated)
...
pub(super) fn call_dataview_method(
    map: &HashMap<String, JsValue>,
    method: &str,
    _args: &[JsValue],
) -> EvalResult {
...
pub(super) fn call_regexp_method(
    map: &HashMap<String, JsValue>,
    method: &str,
    args: &[JsValue],
) -> EvalResult {
...
mod tests {
    ...
    fn abort_controller_unknown_method() {
        ...
        let result = unwrap(call_abort_controller_method(
            &controller,
            "unknownMethod",
            &[],
        ));
    ...
    fn text_encoder_encode() {
        ...
        let result = unwrap(call_text_encoder_method(
            &encoder,
            "encode",
    ...
    fn regexp_test_match() {
        ...
        let result = unwrap(call_regexp_method(
            &regexp,
            "test",
            &[JsValue::String("hello world".into())],
        ));
