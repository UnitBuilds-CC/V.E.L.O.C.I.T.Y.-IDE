...
use super::function::{call_function, call_function_with_this, parse_float_js, parse_int_js};
use super::property::*;
use super::signal::*;
...
pub fn call_native(name: &str, args: &[JsValue]) -> EvalResult {
    Ok(match name {
        ...
        "Number.isInteger" => match args.first() {
            Some(JsValue::Number(n)) => JsValue::Boolean(n.is_finite() && n.fract() == 0.0),
            _ => JsValue::Boolean(false),
        },
        "Number.isSafeInteger" => match args.first() {
            Some(JsValue::Number(n)) => {
                JsValue::Boolean(n.is_finite() && n.fract() == 0.0 && n.abs() <= 9007199254740991.0)
            }
            _ => JsValue::Boolean(false),
        },
        ...
        "Math.pow" => {
            let b = args.first().map(to_number).unwrap_or(0.0);
            let e = args.get(1).map(to_number).unwrap_or(0.0);
            JsValue::Number(b.powf(e))
        }
        ...
        "Math.fround" => {
            JsValue::Number(args.first().map(to_number).unwrap_or(f64::NAN) as f32 as f64)
        }
        "Math.clz32" => {
            let n = args.first().map(to_number).unwrap_or(0.0);
            let u = if n.is_finite() { n as i64 as u32 } else { 0 };
            JsValue::Number(u.leading_zeros() as f64)
        }
        ...
        "Object.keys" => match args.first() {
            Some(obj) => {
                JsValue::Array(own_keys_of(obj).into_iter().map(JsValue::String).collect())
            }
            None => JsValue::Array(Vec::new()),
        },
        "Object.values" => match args.first() {
            Some(obj) => JsValue::Array(
                own_keys_of(obj)
                    .into_iter()
                    .map(|k| get_property(obj, &k))
                    .collect(),
            ),
            None => JsValue::Array(Vec::new()),
        },
        "Object.entries" => match args.first() {
            Some(obj) => JsValue::Array(
                own_keys_of(obj)
                    .into_iter()
                    .map(|k| {
                        JsValue::Array(vec![JsValue::String(k.clone()), get_property(obj, &k)])
                    })
                    .collect(),
            ),
            None => JsValue::Array(Vec::new()),
        ...
        "Object.fromEntries" => {
            ...
            let entries = match args.first() {
                ...
                Some(JsValue::Object(m)) => {
                    if let Some(JsValue::Array(items)) = m.get("__entries__") {
                        items.clone()
                    } else {
                        Vec::new()
                    }
        ...
        "Object.assign" => {
            let mut target = if let Some(JsValue::Object(m)) = args.first() {
                m.clone()
            } else {
                HashMap::new()
            };
            for src in args.iter().skip(1) {
                if let JsValue::Object(m) = src {
                    target.extend(m.iter().map(|(k, v)| (k.clone(), v.clone())));
                }
            }
        ...
        "Object.hasOwn" => {
            ...
            let has = match args.first() {
                ...
                Some(JsValue::Array(arr)) => {
                    key == "length" || key.parse::<usize>().map(|i| i < arr.len()).unwrap_or(false)
                }
                _ => false,
            };
            JsValue::Boolean(has)
        ...
        "Object.setPrototypeOf" => {
            ...
            match args.get(1) {
                Some(JsValue::Null) | None => {
                    target.remove("__proto__");
                }
                Some(proto) => {
                    target.insert("__proto__".to_string(), proto.clone());
                }
            }
            JsValue::Object(target)
        }
                ...
                Some(JsValue::Object(map)) => match map.get(&prop) {
                        ...
                        if desc.get("__accessor__") == Some(&JsValue::Boolean(true)) =>
                    {
                        ...
                        if let Some(g) = desc.get("get") {
                            out.insert("get".to_string(), g.clone());
                        }
                        if let Some(s) = desc.get("set") {
                            out.insert("set".to_string(), s.clone());
                        }
                        JsValue::Object(out)
                    }
                    Some(val) => {
                    ...
                    let desc = match map.get(&key) {
                            ...
                            if d.get("__accessor__") == Some(&JsValue::Boolean(true)) =>
                        {
                            ...
                            if let Some(g) = d.get("get") {
                                acc.insert("get".to_string(), g.clone());
                            }
                            if let Some(s) = d.get("set") {
                                acc.insert("set".to_string(), s.clone());
                            }
                            JsValue::Object(acc)
                        }
                        Some(val) => {
                ...
                _ => match m.get("length") {
                    Some(len_val) => {
                        ...
                        JsValue::Array(
                            (0..len)
                                .map(|i| {
                                    m.get(&i.to_string()).cloned().unwrap_or(JsValue::Undefined)
                                })
                                .collect(),
                        )
                    }
        ...
        "console.log" => {
            push_console("log", args.to_vec());
            return Ok(JsValue::Undefined);
        }
        "console.warn" => {
            push_console("warn", args.to_vec());
            return Ok(JsValue::Undefined);
        }
        "console.error" => {
            push_console("error", args.to_vec());
            return Ok(JsValue::Undefined);
        }
        "console.info" => {
            push_console("info", args.to_vec());
            return Ok(JsValue::Undefined);
        }
        "console.debug" => {
            push_console("debug", args.to_vec());
            return Ok(JsValue::Undefined);
        }
        "console.assert" => {
            let cond = args.first().map(to_boolean).unwrap_or(false);
            if !cond {
                push_console("assert", args.iter().skip(1).cloned().collect());
            }
            return Ok(JsValue::Undefined);
        }
        "console.count" => {
        ...
        "console.timeEnd" => {
            ...
            push_console(
                "timeEnd",
                vec![JsValue::String(format!("{}: {:.3}ms", label, elapsed))],
            );
            return Ok(JsValue::Undefined);
        }
        "console.table" => {
        ...
        "console.trace" => {
            push_console("trace", args.to_vec());
            return Ok(JsValue::Undefined);
        }
        "console.group" => {
            push_console("group", args.to_vec());
            return Ok(JsValue::Undefined);
        }
        "console.groupEnd" => {
            push_console("groupEnd", vec![]);
            return Ok(JsValue::Undefined);
        }
        "console.clear" => {
        ...
        "structuredClone" => args.first().cloned().unwrap_or(JsValue::Undefined),
        "queueMicrotask" | "requestAnimationFrame" | "requestIdleCallback" => JsValue::Number(0.0),
        "__noop__" => JsValue::Undefined,
        "__promise_resolve__" => {
            let val = args.first().cloned().unwrap_or(JsValue::Undefined);
            PROMISE_CAPTURE.with(|cap| {
                *cap.borrow_mut() = Some((false, val));
            });
            JsValue::Undefined
        }
        "__promise_reject__" => {
                ...
                for p in promises {
                    ...
                    if let JsValue::Object(m) = p {
                        if let Some(rejected) = m.get("__rejected__") {
                            ...
                            entry.insert(
                                "value".to_string(),
                                m.get("__resolved__").cloned().unwrap_or(JsValue::Undefined),
                            );
                        }
                    } else {
                        entry.insert(
        ...
        "Reflect.apply" => {
            ...
            call_function_with_this(&target, &call_args, &Scope::new_global(), Some(this_arg))
        ...
        "Reflect.construct" => {
            ...
            match &target {
                JsValue::Object(class_map)
                    if class_map.get("__type__").map(to_string).as_deref() == Some("class") =>
                {
                    call_class_constructor(class_map, &call_args, &Scope::new_global())
                        .unwrap_or(JsValue::Undefined)
                }
                JsValue::Function {
                    params,
                    ...
                    for (i, p) in params.iter().enumerate() {
                        Scope::declare(
                            &call_scope,
                            p,
                            call_args.get(i).cloned().unwrap_or(JsValue::Undefined),
                        );
                    }
                ...
                _ => call_function(&target, &call_args, &Scope::new_global())
                    .unwrap_or(JsValue::Undefined),
            }
        }
        // Timer APIs
        ...
        "matchMedia" => {
            super::web_platform::match_media(&args.first().map(to_string).unwrap_or_default())
        }
        "createImageBitmap" => {
...
pub fn json_stringify(val: &JsValue, replacer: Option<&[String]>) -> String {
    match val {
        ...
        JsValue::Function { .. } | JsValue::NativeFunction(_) | JsValue::Proxy { .. } => {
            "null".to_string()
        }
    }
}
                    ...
                    .filter_map(|key| {
                        map.get(key).and_then(|v| match v {
                            ...
                            other => Some(format!(
                                ...
                                pad,
                                json_escape_string(key),
                                json_stringify_pretty(other, indent, depth + 1, replacer)
                            )),
                        })
                    })
                    .collect()
...
mod tests {
    ...
    fn json_stringify_primitives() {
        ...
        assert_eq!(
            json_stringify(&JsValue::String("hi".into()), None),
            r#""hi""#
        );
    }
    ...
    fn json_stringify_non_finite_number_is_null() {
        ...
        assert_eq!(
            json_stringify(&JsValue::Number(f64::INFINITY), None),
            "null"
        );
    }
