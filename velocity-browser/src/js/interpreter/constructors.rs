...
use super::ast::*;
use super::coercion::*;
use super::eval::{call_class_constructor, eval_expr_node, eval_stmt, PROMISE_CAPTURE};
use super::function::*;
use super::intl::make_intl_locale;
use super::lexer::lex;
use super::parser::Parser;
use super::signal::*;
use super::web_apis2::{make_event_target, make_message_channel};
...
pub(super) fn eval_new(callee: &Expr, args: &[Expr], scope: &ScopeRef) -> EvalResult {
    ...
    match name.or(intl_name.as_deref()) {
        ...
        Some("WeakMap") => {
            ...
            map.insert(
                "__type__".to_string(),
                JsValue::String("WeakMap".to_string()),
            );
            let mut kvs = Vec::new();
            if let Some(JsValue::Array(entries)) = evaluated_args.first() {
                for entry in entries {
        ...
        Some("WeakSet") => {
            ...
            map.insert(
                "__type__".to_string(),
                JsValue::String("WeakSet".to_string()),
            );
            let mut items = Vec::new();
            if let Some(JsValue::Array(init)) = evaluated_args.first() {
                for v in init {
        ...
        Some("Date") => {
            ...
            let ts = if let Some(arg) = evaluated_args.first() {
                match arg {
                    JsValue::Number(n) => *n,
                    JsValue::String(s) => s.parse::<f64>().unwrap_or(0.0),
            ...
            map.insert("__value__".to_string(), JsValue::Number(ts));
            Ok(JsValue::Object(map))
        }
        ...
        Some("Promise") => {
            ...
            if let Some(executor) = evaluated_args.first() {
                ...
                match captured {
                    ...
                    Some((true, reason)) => {
                        map.insert("__rejected__".to_string(), reason);
                    }
                    None => {
        ...
        Some("RegExp") => {
            ...
            map.insert(
                "__type__".to_string(),
                JsValue::String("RegExp".to_string()),
            );
            let source = evaluated_args.first().map(to_string).unwrap_or_default();
            map.insert("source".to_string(), JsValue::String(source));
            let flags = evaluated_args.get(1).map(to_string).unwrap_or_default();
            let has_g = flags.contains('g');
            let has_i = flags.contains('i');
            let has_m = flags.contains('m');
            map.insert("flags".to_string(), JsValue::String(flags));
            map.insert("global".to_string(), JsValue::Boolean(has_g));
            map.insert("ignoreCase".to_string(), JsValue::Boolean(has_i));
            map.insert("multiline".to_string(), JsValue::Boolean(has_m));
            Ok(JsValue::Object(map))
        }
        Some("Function") => {
            let body_str = evaluated_args.last().map(to_string).unwrap_or_default();
            let params: Vec<String> = if evaluated_args.len() > 1 {
                evaluated_args[..evaluated_args.len() - 1]
                    .iter()
                    .map(to_string)
                    .collect()
            } else {
        ...
        Some("Proxy") => {
            let target = evaluated_args
                .first()
                .cloned()
                .unwrap_or(JsValue::Object(HashMap::new()));
            let handler = evaluated_args
                .get(1)
                .cloned()
                .unwrap_or(JsValue::Object(HashMap::new()));
            Ok(JsValue::Proxy {
        ...
        Some("URL") => {
            ...
            let base = evaluated_args.get(1).and_then(|v| {
            ...
            match super::browser_env::make_url(&url_str, base) {
                Ok(url) => Ok(url),
                Err(msg) => Err(Signal::Throw(JsValue::String(format!(
                    "TypeError: {}",
                    msg
                )))),
            }
        ...
        Some("AbortController") => {
            ...
            map.insert(
                "__type__".to_string(),
                JsValue::String("AbortController".to_string()),
            );
            let mut signal = HashMap::new();
            signal.insert(
                "__type__".to_string(),
                JsValue::String("AbortSignal".to_string()),
            );
        ...
        Some("TextEncoder") => {
            ...
            map.insert(
                "__type__".to_string(),
                JsValue::String("TextEncoder".to_string()),
            );
        ...
        Some("TextDecoder") => {
            ...
            map.insert(
                "__type__".to_string(),
                JsValue::String("TextDecoder".to_string()),
            );
            map.insert("encoding".to_string(), JsValue::String("utf-8".to_string()));
            Ok(JsValue::Object(map))
        }
        Some("Response") => {
            ...
            map.insert(
                "__type__".to_string(),
                JsValue::String("Response".to_string()),
            );
            let body = evaluated_args.first().map(to_string).unwrap_or_default();
            map.insert("__body__".to_string(), JsValue::String(body));
            map.insert("status".to_string(), JsValue::Number(200.0));
            map.insert("statusText".to_string(), JsValue::String("OK".to_string()));
            map.insert("headers".to_string(), JsValue::Object(HashMap::new()));
            Ok(JsValue::Object(map))
        }
        Some("Blob") => {
            let mut map = HashMap::new();
            map.insert("__type__".to_string(), JsValue::String("Blob".to_string()));
            let parts = evaluated_args
                .first()
                .cloned()
                .unwrap_or(JsValue::Undefined);
            let data = match &parts {
                JsValue::Array(arr) => {
                    let mut bytes: Vec<JsValue> = Vec::new();
                    for p in arr {
                        match p {
                            JsValue::String(s) => bytes
                                .extend(s.as_bytes().iter().map(|b| JsValue::Number(*b as f64))),
                            JsValue::Array(b) => bytes.extend(
                                b.iter()
                                    .filter_map(|v| {
            ...
            map.insert("__data__".to_string(), data);
            let options = evaluated_args.get(1);
            let mime = if let Some(JsValue::Object(opts)) = options {
                opts.get("type").map(to_string).unwrap_or_default()
            } else {
                String::new()
            };
            map.insert("__mime__".to_string(), JsValue::String(mime));
            Ok(JsValue::Object(map))
        }
        Some("Uint8Array")
        | Some("Int8Array")
        ...
        | Some("Uint8ClampedArray") => {
            ...
            let size = evaluated_args.first().map(to_number).unwrap_or(0.0) as usize;
            let data: Vec<JsValue> = (0..size).map(|_| JsValue::Number(0.0)).collect();
            map.insert("__data__".to_string(), JsValue::Array(data));
            Ok(JsValue::Object(map))
        }
        Some("DataView") => {
            let mut map = HashMap::new();
            map.insert(
                "__type__".to_string(),
                JsValue::String("DataView".to_string()),
            );
            let buffer = evaluated_args
                .first()
                .cloned()
                .unwrap_or(JsValue::Undefined);
            map.insert("__buffer__".to_string(), buffer);
            let byte_offset = evaluated_args.get(1).map(to_number).unwrap_or(0.0);
            map.insert("__byteOffset__".to_string(), JsValue::Number(byte_offset));
            let byte_length = evaluated_args.get(2).map(to_number).unwrap_or(0.0);
            map.insert("__byteLength__".to_string(), JsValue::Number(byte_length));
            Ok(JsValue::Object(map))
        }
        Some("ArrayBuffer") => {
            let mut map = HashMap::new();
        ...
        Some("Intl.Segmenter") => {
            ...
            map.insert(
                "__type__".to_string(),
                JsValue::String("Intl.Segmenter".to_string()),
            );
            let options = evaluated_args.first().cloned();
            if let Some(JsValue::Object(opts)) = options {
                for (k, v) in opts {
                    if k == "locale" || k == "granularity" {
                        map.insert(k, v);
                    }
                }
            }
        ...
        Some("Intl.Collator") => {
            ...
            map.insert(
                "__type__".to_string(),
                JsValue::String("Intl.Collator".to_string()),
            );
            let options = evaluated_args.first().cloned();
            if let Some(JsValue::Object(opts)) = options {
                for (k, v) in opts {
                    if k == "locale" || k == "sensitivity" {
                        map.insert(k, v);
                    }
                }
            }
        ...
        Some("Intl.NumberFormat") => {
            ...
            map.insert(
                "__type__".to_string(),
                JsValue::String("Intl.NumberFormat".to_string()),
            );
            let options = evaluated_args.first().cloned();
            if let Some(JsValue::Object(opts)) = options {
                for (k, v) in opts {
        ...
        Some("Intl.DateTimeFormat") => {
            ...
            map.insert(
                "__type__".to_string(),
                JsValue::String("Intl.DateTimeFormat".to_string()),
            );
            let options = evaluated_args.first().cloned();
            if let Some(JsValue::Object(opts)) = options {
                for (k, v) in opts {
        ...
        Some("Intl.PluralRules") => {
            ...
            map.insert(
                "__type__".to_string(),
                JsValue::String("Intl.PluralRules".to_string()),
            );
            let options = evaluated_args.first().cloned();
            if let Some(JsValue::Object(opts)) = options {
                for (k, v) in opts {
        ...
        Some("Intl.RelativeTimeFormat") => {
            ...
            map.insert(
                "__type__".to_string(),
                JsValue::String("Intl.RelativeTimeFormat".to_string()),
            );
            let options = evaluated_args.first().cloned();
            if let Some(JsValue::Object(opts)) = options {
                for (k, v) in opts {
        ...
        Some("Intl.DurationFormat") => {
            ...
            map.insert(
                "__type__".to_string(),
                JsValue::String("Intl.DurationFormat".to_string()),
            );
            let options = evaluated_args.first().cloned();
            if let Some(JsValue::Object(opts)) = options {
                for (k, v) in opts {
        ...
        Some("Intl.ListFormat") => {
            ...
            map.insert(
                "__type__".to_string(),
                JsValue::String("Intl.ListFormat".to_string()),
            );
            let options = evaluated_args.first().cloned();
            if let Some(JsValue::Object(opts)) = options {
                for (k, v) in opts {
        ...
        Some("Intl.DisplayNames") => {
            ...
            map.insert(
                "__type__".to_string(),
                JsValue::String("Intl.DisplayNames".to_string()),
            );
            let options = evaluated_args.first().cloned();
            if let Some(JsValue::Object(opts)) = options {
                for (k, v) in opts {
        ...
        Some("WeakRef") => {
            ...
            map.insert(
                "__type__".to_string(),
                JsValue::String("WeakRef".to_string()),
            );
            map.insert(
        ...
        Some("FinalizationRegistry") => {
            ...
            map.insert(
                "__type__".to_string(),
                JsValue::String("FinalizationRegistry".to_string()),
            );
            map.insert(
                "__callback__".to_string(),
                evaluated_args
                    .first()
                    .cloned()
                    .unwrap_or(JsValue::Undefined),
        ...
        Some("Event") => {
            ...
            Ok(super::browser_env::make_event(
                &event_type,
        ...
        Some("CustomEvent") => {
            let event_type = evaluated_args.first().map(to_string).unwrap_or_default();
            Ok(super::browser_env::make_custom_event(
                &event_type,
                evaluated_args.get(1),
            ))
        }
        ...
        Some("IntersectionObserver") => {
            let callback = evaluated_args
                ...
                .cloned()
                .unwrap_or(JsValue::Undefined);
            Ok(super::web_platform::make_intersection_observer(
                callback,
                evaluated_args.get(1),
            ))
        }
        ...
        Some("WebSocket") => {
            ...
            Ok(super::web_platform::make_web_socket(
                &url,
                evaluated_args.get(1),
            ))
        }
        ...
        Some("TransformStream") => Ok(super::streams::make_transform_stream(
            evaluated_args.first(),
        )),
        Some("CountQueuingStrategy") => {
            let hwm = evaluated_args
                .first()
                .and_then(|v| {
                    if let JsValue::Object(m) = v {
                        m.get("highWaterMark").map(to_number)
                    } else {
                        None
                    }
                })
        ...
        Some("DOMException") => {
            ...
            map.insert(
                "__type__".to_string(),
                JsValue::String("DOMException".to_string()),
            );
        ...
        Some("Comment") => {
            ...
            map.insert(
                "__type__".to_string(),
                JsValue::String("Comment".to_string()),
            );
            map.insert("data".to_string(), JsValue::String(data.clone()));
        ...
        Some("AudioContext") | Some("webkitAudioContext") => {
            ...
            ctx.insert(
                "__type__".to_string(),
                JsValue::String("AudioContext".to_string()),
            );
        ...
        Some("SpeechSynthesisUtterance") => {
            ...
            utt.insert(
                "__type__".to_string(),
                JsValue::String("SpeechSynthesisUtterance".to_string()),
            );
        ...
        Some("UIEvent") => {
            ...
            Ok(super::browser_env::make_typed_event(
                "UIEvent",
                &et,
                evaluated_args.get(1),
                &[("detail", JsValue::Number(0.0)), ("view", JsValue::Null)],
            ))
        }
        Some("FocusEvent") => {
            let et = evaluated_args.first().map(to_string).unwrap_or_default();
        ...
        Some("DragEvent") => {
            ...
            dt.insert(
                "__type__".to_string(),
                JsValue::String("DataTransfer".to_string()),
            );
            dt.insert(
                "dropEffect".to_string(),
                JsValue::String("none".to_string()),
            );
            dt.insert(
                "effectAllowed".to_string(),
                JsValue::String("uninitialized".to_string()),
            );
        ...
        Some("DataTransferItem") => {
            ...
            item.insert(
                "__type__".to_string(),
                JsValue::String("DataTransferItem".to_string()),
            );
        ...
        _ => {
            ...
            if let JsValue::Object(class_map) = &callee_val {
                let is_class = class_map
                    .get("__type__")
                    .map(|v| matches!(v, JsValue::String(s) if s == "class"))
                    .unwrap_or(false);
                if is_class {
                ...
                match eval_stmt(body, &call_scope) {
                    ...
                    Err(Signal::Return(v)) => {
                        if matches!(v, JsValue::Object(_)) {
                            ...
                            Ok(Scope::resolve(&call_scope, "this")
                                .unwrap_or(JsValue::Object(HashMap::new())))
                        }
                    }
                    Err(e) => Err(e),
