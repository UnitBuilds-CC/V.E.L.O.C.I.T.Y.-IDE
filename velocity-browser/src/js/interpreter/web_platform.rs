...
fn entry_to_js(e: &PerfEntry) -> JsValue {
    ...
    map.insert("entryType".to_string(), JsValue::String(e.entry_type.clone()));
...
pub(super) fn make_performance() -> JsValue {
    let mut map = HashMap::new();
    map.insert("__type__".to_string(), JsValue::String("Performance".to_string()));
    let origin = PERF_TIME_ORIGIN.with(|o| {
        let v = o.get();
        if v == 0.0 {
            let now = std::time::SystemTime::now()
...
pub(super) fn call_performance_method(method: &str, args: &[JsValue]) -> JsValue {
    match method {
        ...
        "mark" => {
            ...
            let time = perf_now();
            PERF_MARKS.with(|m| { m.borrow_mut().insert(name.clone(), time); });
            PERF_ENTRIES.with(|e| {
                e.borrow_mut().push(PerfEntry { name, entry_type: "mark".to_string(), start_time: time, duration: 0.0 });
            });
            JsValue::Undefined
        }
        "measure" => {
            let name = args.first().map(crate::js::interpreter::coercion::to_string).unwrap_or_default();
            let start_mark = args.get(1).map(crate::js::interpreter::coercion::to_string).unwrap_or_default();
            let end_mark = args.get(2).map(crate::js::interpreter::coercion::to_string).unwrap_or_default();
            let (start_time, duration) = PERF_MARKS.with(|m| {
                let marks = m.borrow();
                let s = marks.get(&start_mark).copied().unwrap_or(0.0);
                let e = if end_mark.is_empty() { perf_now() } else { marks.get(&end_mark).copied().unwrap_or_else(perf_now) };
                (s, e - s)
            });
            PERF_ENTRIES.with(|e| {
                e.borrow_mut().push(PerfEntry { name, entry_type: "measure".to_string(), start_time, duration });
            });
            JsValue::Undefined
        }
        "getEntries" => {
        ...
        "getEntriesByName" => {
            let name = args.first().map(crate::js::interpreter::coercion::to_string).unwrap_or_default();
            let entries = PERF_ENTRIES.with(|e| e.borrow().iter().filter(|p| p.name == name).map(entry_to_js).collect());
            JsValue::Array(entries)
        }
        "getEntriesByType" => {
            let t = args.first().map(crate::js::interpreter::coercion::to_string).unwrap_or_default();
            let entries = PERF_ENTRIES.with(|e| e.borrow().iter().filter(|p| p.entry_type == t).map(entry_to_js).collect());
            JsValue::Array(entries)
        }
        "clearMarks" => {
            PERF_MARKS.with(|m| { m.borrow_mut().clear(); });
            PERF_ENTRIES.with(|e| { e.borrow_mut().retain(|p| p.entry_type != "mark"); });
            JsValue::Undefined
        }
        "clearMeasures" => {
            PERF_ENTRIES.with(|e| { e.borrow_mut().retain(|p| p.entry_type != "measure"); });
            JsValue::Undefined
        }
...
pub(super) fn make_history() -> JsValue {
    let mut map = HashMap::new();
    map.insert("__type__".to_string(), JsValue::String("History".to_string()));
    ...
    let state = HISTORY_STACK.with(|s| {
        ...
        s.borrow().get(idx).map(|e| e.state.clone()).unwrap_or(JsValue::Null)
    });
...
pub(super) fn call_history_method(method: &str, args: &[JsValue]) -> JsValue {
    match method {
        "pushState" => {
            ...
            let url = args.get(2).map(crate::js::interpreter::coercion::to_string).unwrap_or_default();
        ...
        "forward" => {
            HISTORY_STACK.with(|s| {
                ...
                HISTORY_INDEX.with(|i| {
                    let idx = i.get();
                    if idx + 1 < len { i.set(idx + 1); }
                });
            });
            JsValue::Undefined
        }
        "go" => {
            let delta = args.first().map(crate::js::interpreter::coercion::to_number).unwrap_or(0.0) as i32;
            HISTORY_STACK.with(|s| {
...
pub(super) fn make_intersection_observer(callback: JsValue, _options: Option<&JsValue>) -> JsValue {
    let mut map = HashMap::new();
    map.insert("__type__".to_string(), JsValue::String("IntersectionObserver".to_string()));
    map.insert("__callback__".to_string(), callback);
    map.insert("__targets__".to_string(), JsValue::Array(Vec::new()));
    JsValue::Object(map)
}

pub(super) fn call_intersection_observer_method(map: &HashMap<String, JsValue>, method: &str, args: &[JsValue]) -> JsValue {
    match method {
        "observe" => {
            let mut m = map.clone();
...
pub(super) fn make_resize_observer(callback: JsValue) -> JsValue {
    let mut map = HashMap::new();
    map.insert("__type__".to_string(), JsValue::String("ResizeObserver".to_string()));
    map.insert("__callback__".to_string(), callback);
    map.insert("__targets__".to_string(), JsValue::Array(Vec::new()));
    JsValue::Object(map)
}

pub(super) fn call_resize_observer_method(map: &HashMap<String, JsValue>, method: &str, args: &[JsValue]) -> JsValue {
    match method {
        "observe" => {
            let mut m = map.clone();
            let target = args.first().cloned().unwrap_or(JsValue::Undefined);
...
pub(super) fn make_web_socket(url: &str, _protocols: Option<&JsValue>) -> JsValue {
    let mut map = HashMap::new();
    map.insert("__type__".to_string(), JsValue::String("WebSocket".to_string()));
...
pub(super) fn call_web_socket_method(map: &HashMap<String, JsValue>, method: &str, args: &[JsValue]) -> JsValue {
    match method {
        "send" => {
            let mut m = map.clone();
...
pub(super) fn get_computed_style(_element: &JsValue, _pseudo: Option<&JsValue>) -> JsValue {
    let mut map = HashMap::new();
    map.insert("__type__".to_string(), JsValue::String("CSSStyleDeclaration".to_string()));
    ...
    let defaults = [
        ...
        ("lineHeight", "normal"), ("margin", "0px"), ("padding", "0px"),
        ("border", "0px none rgb(0, 0, 0)"), ("width", "auto"), ("height", "auto"),
        ("position", "static"), ("top", "auto"), ("left", "auto"),
        ("right", "auto"), ("bottom", "auto"), ("zIndex", "auto"),
        ("overflow", "visible"), ("cursor", "auto"), ("textAlign", "start"),
        ("textDecoration", "none"), ("textTransform", "none"),
        ("boxSizing", "content-box"), ("flexDirection", "row"),
...
pub(super) fn call_css_style_declaration_method(map: &HashMap<String, JsValue>, method: &str, args: &[JsValue]) -> JsValue {
    match method {
        "getPropertyValue" => {
            let prop = args.first().map(crate::js::interpreter::coercion::to_string).unwrap_or_default();
            // Convert kebab-case to camelCase for lookup.
            let camel = kebab_to_camel(&prop);
            map.get(&camel).cloned().unwrap_or(JsValue::String(String::new()))
        }
        "getPropertyPriority" => JsValue::String(String::new()),
        "item" => {
            let idx = args.first().map(crate::js::interpreter::coercion::to_number).unwrap_or(0.0) as usize;
            let keys: Vec<&String> = {
                let mut ks: Vec<&String> = map.keys().filter(|k| !k.starts_with("__")).collect();
                ks.sort();
                ks
            };
            keys.get(idx).map(|k| JsValue::String((*k).clone())).unwrap_or(JsValue::String(String::new()))
        }
        _ => JsValue::Undefined,
    }
}
...
fn kebab_to_camel(s: &str) -> String {
    ...
    let mut upper = false;
    for c in s.chars() {
        if c == '-' { upper = true; } else if upper { out.push(c.to_ascii_uppercase()); upper = false; } else { out.push(c); }
    }
    out
}
...
pub(super) fn match_media(query: &str) -> JsValue {
    let mut map = HashMap::new();
    map.insert("__type__".to_string(), JsValue::String("MediaQueryList".to_string()));
...
fn eval_media_query(query: &str) -> bool {
    ...
    if q.contains("prefers-color-scheme: dark") { return false; }
    if q.contains("prefers-color-scheme: light") { return true; }
    if q.contains("prefers-reduced-motion: reduce") { return false; }
    if q.contains("prefers-reduced-motion: no-preference") { return true; }
    if q.contains("prefers-contrast: more") { return false; }
    if q.contains("color-gamut: p3") { return false; }
    if q.contains("color-gamut: srgb") { return true; }
    if q.contains("hover: hover") { return true; }
    if q.contains("hover: none") { return false; }
    if q.contains("pointer: fine") { return true; }
    if q.contains("pointer: coarse") { return false; }
    if q.contains("any-pointer: fine") { return true; }
    // Width queries.
    if let Some(w) = extract_px(&q, "min-width") { return 1280.0 >= w; }
    if let Some(w) = extract_px(&q, "max-width") { return 1280.0 <= w; }
    if let Some(h) = extract_px(&q, "min-height") { return 720.0 >= h; }
    if let Some(h) = extract_px(&q, "max-height") { return 720.0 <= h; }
    if q.contains("screen") { return true; }
    if q.contains("print") { return false; }
    true
}
...
pub(super) fn call_media_query_list_method(map: &HashMap<String, JsValue>, method: &str, _args: &[JsValue]) -> JsValue {
    match method {
        "addListener" | "addEventListener" | "removeListener" | "removeEventListener" => JsValue::Undefined,
...
pub(super) fn make_file_reader() -> JsValue {
    let mut map = HashMap::new();
    map.insert("__type__".to_string(), JsValue::String("FileReader".to_string()));
...
pub(super) fn call_file_reader_method(map: &HashMap<String, JsValue>, method: &str, args: &[JsValue]) -> JsValue {
    match method {
        "readAsText" => {
            let mut m = map.clone();
            let blob = args.first();
        ...
        "readAsDataURL" => {
            ...
            let data_url = format!("data:{};base64,{}", if mime.is_empty() { "application/octet-stream" } else { &mime }, b64);
        ...
        "readAsArrayBuffer" => {
            ...
            let blob = args.first();
            let text = extract_blob_text(blob);
            let bytes: Vec<JsValue> = text.as_bytes().iter().map(|b| JsValue::Number(*b as f64)).collect();
            let mut buf = HashMap::new();
            buf.insert("__type__".to_string(), JsValue::String("ArrayBuffer".to_string()));
...
fn extract_blob_text(blob: Option<&JsValue>) -> String {
    match blob {
        Some(JsValue::Object(m)) => {
            if let Some(JsValue::String(s)) = m.get("__data__") { return s.clone(); }
            if let Some(JsValue::Array(bytes)) = m.get("__data__") {
                return bytes.iter().filter_map(|v| if let JsValue::Number(n) = v { Some(*n as u8) } else { None }).collect::<Vec<u8>>().iter().map(|b| *b as char).collect();
            }
            String::new()
        }
...
fn extract_blob_mime(blob: Option<&JsValue>) -> String {
    match blob {
        Some(JsValue::Object(m)) => m.get("__mime__").map(crate::js::interpreter::coercion::to_string).unwrap_or_default(),
        _ => String::new(),
    }
}
...
pub(super) fn call_subtle_method(method: &str, args: &[JsValue]) -> JsValue {
    match method {
        "digest" => {
            ...
            let algo = args.first().map(crate::js::interpreter::coercion::to_string).unwrap_or_default();
            let data = args.get(1);
            let hash = simple_hash(data);
            let len = if algo.to_uppercase().contains("SHA-256") { 32 }
                else if algo.to_uppercase().contains("SHA-384") { 48 }
                else if algo.to_uppercase().contains("SHA-512") { 64 }
                else if algo.to_uppercase().contains("SHA-1") { 20 }
                else { 32 };
            let bytes: Vec<JsValue> = (0..len).map(|i| JsValue::Number(hash[i % hash.len()] as f64)).collect();
            let mut buf = HashMap::new();
            buf.insert("__type__".to_string(), JsValue::String("ArrayBuffer".to_string()));
            buf.insert("__data__".to_string(), JsValue::Array(bytes));
            JsValue::Object(buf)
        ...
        "encrypt" | "decrypt" => {
            ...
            let result: Vec<JsValue> = data_bytes.iter().enumerate()
        ...
        "sign" | "verify" => {
            ...
            if method == "verify" { return JsValue::Boolean(true); }
            let data = args.get(2);
            let hash = simple_hash(data);
        ...
        "importKey" => {
            let mut map = HashMap::new();
            map.insert("__type__".to_string(), JsValue::String("CryptoKey".to_string()));
        ...
        "generateKey" => {
            let mut map = HashMap::new();
            map.insert("__type__".to_string(), JsValue::String("CryptoKeyPair".to_string()));
            let mut pub_key = HashMap::new();
            pub_key.insert("__type__".to_string(), JsValue::String("CryptoKey".to_string()));
            pub_key.insert("type".to_string(), JsValue::String("public".to_string()));
            let mut priv_key = HashMap::new();
            priv_key.insert("__type__".to_string(), JsValue::String("CryptoKey".to_string()));
        ...
        "deriveBits" | "deriveKey" => {
            ...
            let bytes: Vec<JsValue> = (0..len).map(|i| JsValue::Number((i * 7 + 13) as f64)).collect();
            buf.insert("__data__".to_string(), JsValue::Array(bytes));
            JsValue::Object(buf)
        }
        _ => JsValue::Undefined,
    ...
    match data {
        Some(JsValue::Object(m)) => {
            if let Some(JsValue::Array(arr)) = m.get("__data__") {
                return arr.iter().filter_map(|v| if let JsValue::Number(n) = v { Some(*n as u8) } else { None }).collect();
            }
        }
...
fn extract_key_bytes(key: Option<&JsValue>) -> Vec<u8> {
    match key {
        Some(JsValue::Object(m)) => {
            if let Some(JsValue::String(s)) = m.get("__key__") { return s.as_bytes().to_vec(); }
...
pub(super) fn make_css_style_sheet() -> JsValue {
    let mut map = HashMap::new();
    map.insert("__type__".to_string(), JsValue::String("CSSStyleSheet".to_string()));
    map.insert("__rules__".to_string(), JsValue::Array(Vec::new()));
    JsValue::Object(map)
}

pub(super) fn call_css_style_sheet_method(map: &HashMap<String, JsValue>, method: &str, args: &[JsValue]) -> JsValue {
    match method {
        "replace" | "replaceSync" => {
            let mut m = map.clone();
            let css = args.first().map(crate::js::interpreter::coercion::to_string).unwrap_or_default();
            let rules: Vec<JsValue> = css.split('}').filter(|s| s.contains('{')).map(|s| JsValue::String(format!("{}}}", s.trim()))).collect();
        ...
        "insertRule" => {
            let mut m = map.clone();
            let rule = args.first().map(crate::js::interpreter::coercion::to_string).unwrap_or_default();
            if let Some(JsValue::Array(rules)) = m.get_mut("__rules__") {
                let idx = args.get(1).map(crate::js::interpreter::coercion::to_number).unwrap_or(rules.len() as f64) as usize;
...
pub(super) fn make_dom_rect(x: f64, y: f64, w: f64, h: f64) -> JsValue {
    let mut map = HashMap::new();
    map.insert("__type__".to_string(), JsValue::String("DOMRect".to_string()));
...
pub(super) fn call_dom_rect_method(map: &HashMap<String, JsValue>, method: &str, _args: &[JsValue]) -> JsValue {
    match method {
...
pub(super) fn make_dom_matrix(a: f64, b: f64, c: f64, d: f64, e: f64, f: f64) -> JsValue {
    let mut map = HashMap::new();
    map.insert("__type__".to_string(), JsValue::String("DOMMatrix".to_string()));
    ...
    map.insert("isIdentity".to_string(), JsValue::Boolean(a == 1.0 && b == 0.0 && c == 0.0 && d == 1.0 && e == 0.0 && f == 0.0));
    JsValue::Object(map)
}
...
pub(super) fn call_navigator_method(method: &str, args: &[JsValue]) -> JsValue {
    match method {
        "clipboard" => {
            let mut map = HashMap::new();
            map.insert("__type__".to_string(), JsValue::String("Clipboard".to_string()));
            JsValue::Object(map)
        }
        "permissions" => {
            let mut map = HashMap::new();
            map.insert("__type__".to_string(), JsValue::String("Permissions".to_string()));
            JsValue::Object(map)
        }
        "geolocation" => {
            let mut map = HashMap::new();
            map.insert("__type__".to_string(), JsValue::String("Geolocation".to_string()));
            JsValue::Object(map)
        }
        ...
        "getBattery" => {
            ...
            battery.insert("__type__".to_string(), JsValue::String("BatteryManager".to_string()));
            battery.insert("charging".to_string(), JsValue::Boolean(true));
            battery.insert("chargingTime".to_string(), JsValue::Number(0.0));
            battery.insert("dischargingTime".to_string(), JsValue::Number(f64::INFINITY));
        ...
        "connection" => {
            ...
            conn.insert("__type__".to_string(), JsValue::String("NetworkInformation".to_string()));
            conn.insert("effectiveType".to_string(), JsValue::String("4g".to_string()));
        ...
        "storage" => {
            ...
            storage.insert("__type__".to_string(), JsValue::String("StorageManager".to_string()));
            JsValue::Object(storage)
        }
        "mediaDevices" => {
            let mut md = HashMap::new();
            md.insert("__type__".to_string(), JsValue::String("MediaDevices".to_string()));
            JsValue::Object(md)
        }
        "wakeLock" => {
            let mut wl = HashMap::new();
            wl.insert("__type__".to_string(), JsValue::String("WakeLock".to_string()));
            JsValue::Object(wl)
        }
...
pub(super) fn call_permissions_method(method: &str, args: &[JsValue]) -> JsValue {
    match method {
        "query" => {
            ...
            status.insert("__type__".to_string(), JsValue::String("PermissionStatus".to_string()));
...
pub(super) fn call_geolocation_method(method: &str, args: &[JsValue]) -> JsValue {
    match method {
        "getCurrentPosition" => {
            ...
            position.insert("__type__".to_string(), JsValue::String("GeolocationPosition".to_string()));
...
pub(super) fn call_caches_method(method: &str, args: &[JsValue]) -> JsValue {
    match method {
        "open" => {
            let name = args.first().map(crate::js::interpreter::coercion::to_string).unwrap_or_default();
            CACHES.with(|c| { c.borrow_mut().entry(name.clone()).or_default(); });
            let mut p = HashMap::new();
            p.insert("__type__".to_string(), JsValue::String("Promise".to_string()));
        ...
        "keys" => {
            let keys: Vec<JsValue> = CACHES.with(|c| c.borrow().keys().map(|k| JsValue::String(k.clone())).collect());
            let mut p = HashMap::new();
            p.insert("__type__".to_string(), JsValue::String("Promise".to_string()));
...
pub(super) fn call_cache_method(map: &HashMap<String, JsValue>, method: &str, args: &[JsValue]) -> JsValue {
    let name = map.get("__name__").map(crate::js::interpreter::coercion::to_string).unwrap_or_default();
    match method {
        "put" => {
            let url = args.first().map(crate::js::interpreter::coercion::to_string).unwrap_or_default();
        ...
        "match" => {
            ...
            let result = CACHES.with(|c| {
                c.borrow().get(&name).and_then(|entries| entries.iter().find(|(u, _)| u == &url).map(|(_, r)| r.clone()))
            });
            let mut p = HashMap::new();
            p.insert("__type__".to_string(), JsValue::String("Promise".to_string()));
            p.insert("__resolved__".to_string(), result.unwrap_or(JsValue::Undefined));
            JsValue::Object(p)
        }
        "delete" => {
            let url = args.first().map(crate::js::interpreter::coercion::to_string).unwrap_or_default();
        ...
        "keys" => {
            let keys: Vec<JsValue> = CACHES.with(|c| {
                c.borrow().get(&name).map(|entries| entries.iter().map(|(u, _)| JsValue::String(u.clone())).collect()).unwrap_or_default()
            });
            let mut p = HashMap::new();
            p.insert("__type__".to_string(), JsValue::String("Promise".to_string()));
            p.insert("__resolved__".to_string(), JsValue::Array(keys));
...
pub(super) fn make_worker(script_url: &str) -> JsValue {
    let mut map = HashMap::new();
    map.insert("__type__".to_string(), JsValue::String("Worker".to_string()));
    map.insert("__script_url__".to_string(), JsValue::String(script_url.to_string()));
    map.insert("__posted__".to_string(), JsValue::Array(Vec::new()));
    map.insert("__terminated__".to_string(), JsValue::Boolean(false));
    JsValue::Object(map)
}

pub(super) fn call_worker_method(map: &HashMap<String, JsValue>, method: &str, args: &[JsValue]) -> JsValue {
    match method {
        "postMessage" => {
            let mut m = map.clone();
...
pub(super) fn make_shared_worker(script_url: &str) -> JsValue {
    let mut map = HashMap::new();
    map.insert("__type__".to_string(), JsValue::String("SharedWorker".to_string()));
    map.insert("__script_url__".to_string(), JsValue::String(script_url.to_string()));
    let mut port = HashMap::new();
    port.insert("__type__".to_string(), JsValue::String("MessagePort".to_string()));
...
pub(super) fn make_service_worker_container() -> JsValue {
    let mut map = HashMap::new();
    map.insert("__type__".to_string(), JsValue::String("ServiceWorkerContainer".to_string()));
    map.insert("ready".to_string(), {
        let mut p = HashMap::new();
        p.insert("__type__".to_string(), JsValue::String("Promise".to_string()));
        let mut reg = HashMap::new();
        reg.insert("__type__".to_string(), JsValue::String("ServiceWorkerRegistration".to_string()));
        reg.insert("scope".to_string(), JsValue::String("https://localhost/".to_string()));
...
pub(super) fn call_service_worker_container_method(method: &str, args: &[JsValue]) -> JsValue {
    match method {
        "register" => {
            let _url = args.first().map(crate::js::interpreter::coercion::to_string).unwrap_or_default();
            let mut p = HashMap::new();
            p.insert("__type__".to_string(), JsValue::String("Promise".to_string()));
            let mut reg = HashMap::new();
            reg.insert("__type__".to_string(), JsValue::String("ServiceWorkerRegistration".to_string()));
        ...
        "getRegistration" | "getRegistrations" => {
            let mut p = HashMap::new();
            p.insert("__type__".to_string(), JsValue::String("Promise".to_string()));
            p.insert("__resolved__".to_string(), if method == "getRegistrations" { JsValue::Array(Vec::new()) } else { JsValue::Undefined });
            JsValue::Object(p)
        }
        "unregister" => {
            let mut p = HashMap::new();
...
pub(super) fn make_performance_observer(callback: JsValue) -> JsValue {
    let mut map = HashMap::new();
    map.insert("__type__".to_string(), JsValue::String("PerformanceObserver".to_string()));
    map.insert("__callback__".to_string(), callback);
    map.insert("__entry_types__".to_string(), JsValue::Array(Vec::new()));
    JsValue::Object(map)
}

pub(super) fn call_performance_observer_method(map: &HashMap<String, JsValue>, method: &str, args: &[JsValue]) -> JsValue {
    match method {
        "observe" => {
            let mut m = map.clone();
...
pub(super) fn make_indexed_db() -> JsValue {
    let mut map = HashMap::new();
    map.insert("__type__".to_string(), JsValue::String("IDBFactory".to_string()));
    JsValue::Object(map)
}
