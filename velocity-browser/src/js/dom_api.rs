...
pub fn eval_dom(tree: &mut DomTree, expr: &str) -> Option<Result<JsValue, String>> {
    ...
    if let Some(arg) = expr.strip_prefix("document.createElement(").and_then(|s| s.strip_suffix(")")) {
        let tag = unquote(arg);
    ...
    if let Some(arg) = expr.strip_prefix("document.createTextNode(").and_then(|s| s.strip_suffix(")")) {
    ...
    if let Some(arg) = expr.strip_prefix("document.querySelectorAll(").and_then(|s| s.strip_suffix(")")) {
        let sel = unquote(arg);
    ...
    if let Some(args) = rest.strip_prefix(".setAttribute(").and_then(|s| s.strip_suffix(")")) {
        if let Some((attr, val)) = args.split_once(',') {
            let attr = unquote(attr);
            let val = unquote(val);
            if let Some(node) = tree.get_node_mut(id) {
                node.attributes.insert(attr.clone(), val.clone());
                return Some(Ok(JsValue::String(format!("Set attribute '{}'='{}'", attr, val))));
            }
        }
    ...
    if let Some(arg) = rest.strip_prefix(".getAttribute(").and_then(|s| s.strip_suffix(")")) {
        let key = unquote(arg);
        return Some(Ok(tree
            .get_node(id)
    ...
    if let Some(arg) = rest.strip_prefix(".removeAttribute(").and_then(|s| s.strip_suffix(")")) {
        let key = unquote(arg);
        if let Some(node) = tree.get_node_mut(id) {
    ...
    if let Some(arg) = rest.strip_prefix(".hasAttribute(").and_then(|s| s.strip_suffix(")")) {
        let key = unquote(arg);
        return Some(Ok(JsValue::Boolean(
            tree.get_node(id).map(|n| n.attributes.contains_key(&key)).unwrap_or(false)
        )));
    }

    // Method: appendChild(child_handle) — expects __node_id__ in call
    if let Some(arg) = rest.strip_prefix(".appendChild(").and_then(|s| s.strip_suffix(")")) {
    ...
    if let Some(arg) = rest.strip_prefix(".removeChild(").and_then(|s| s.strip_suffix(")")) {
        if let Some(child_id) = parse_node_id_arg(arg.trim()) {
    ...
    if let Some(args) = rest.strip_prefix(".insertBefore(").and_then(|s| s.strip_suffix(")")) {
        if let Some((new_arg, ref_arg)) = args.split_once(',') {
            if let (Some(new_id), Some(ref_id)) = (parse_node_id_arg(new_arg.trim()), parse_node_id_arg(ref_arg.trim())) {
    ...
    if let Some(arg) = rest.strip_prefix(".querySelectorAll(").and_then(|s| s.strip_suffix(")")) {
        let sel = unquote(arg);
        let all = tree.query_selector_all(&sel);
        // Filter to descendants of `id`
        let descendants: Vec<JsValue> = all.into_iter()
    ...
    if rest == ".children" {
        ...
        return Some(Ok(JsValue::Array(children.into_iter().map(element_handle).collect())));
    }

    // Property: .childNodes
    if rest == ".childNodes" {
        let children = tree.get_node(id).map(|n| n.children.clone()).unwrap_or_default();
        return Some(Ok(JsValue::Array(children.into_iter().map(element_handle).collect())));
    }

    // Property: .parentNode / .parentElement
    if rest == ".parentNode" || rest == ".parentElement" {
        return Some(Ok(tree.get_node(id)
            .and_then(|n| n.parent)
            .map(element_handle)
            .unwrap_or(JsValue::Null)));
    }
    ...
    if rest == ".nextSibling" || rest == ".nextElementSibling" {
        return Some(Ok(get_sibling(tree, id, 1).map(element_handle).unwrap_or(JsValue::Null)));
    }
    ...
    if let Some(val_part) = rest.strip_prefix(".innerHTML=").or_else(|| rest.strip_prefix(".innerHTML =")) {
    ...
    if let Some(val_part) = rest.strip_prefix(".textContent=").or_else(|| rest.strip_prefix(".textContent =")) {
    ...
    if let Some(class_rest) = rest.strip_prefix(".classList.") {
        if let Some(arg) = class_rest.strip_prefix("add(").and_then(|s| s.strip_suffix(")")) {
            let cls = unquote(arg);
            if let Some(node) = tree.get_node_mut(id) {
                let current = node.attributes.get("class").cloned().unwrap_or_default();
                if !current.split_whitespace().any(|c| c == cls) {
                    let new_class = if current.is_empty() { cls } else { format!("{} {}", current, cls) };
                    node.attributes.insert("class".to_string(), new_class);
                }
            }
            return Some(Ok(JsValue::Undefined));
        }
        if let Some(arg) = class_rest.strip_prefix("remove(").and_then(|s| s.strip_suffix(")")) {
            let cls = unquote(arg);
            if let Some(node) = tree.get_node_mut(id) {
                let current = node.attributes.get("class").cloned().unwrap_or_default();
                let new_class: String = current.split_whitespace().filter(|&c| c != cls).collect::<Vec<_>>().join(" ");
                node.attributes.insert("class".to_string(), new_class);
            }
            return Some(Ok(JsValue::Undefined));
        }
        if let Some(arg) = class_rest.strip_prefix("toggle(").and_then(|s| s.strip_suffix(")")) {
            let cls = unquote(arg);
            if let Some(node) = tree.get_node_mut(id) {
                let current = node.attributes.get("class").cloned().unwrap_or_default();
                let has_it = current.split_whitespace().any(|c| c == cls);
                let new_class = if has_it {
                    current.split_whitespace().filter(|&c| c != cls).collect::<Vec<_>>().join(" ")
                } else if current.is_empty() { cls.clone() } else { format!("{} {}", current, cls) };
                node.attributes.insert("class".to_string(), new_class);
                return Some(Ok(JsValue::Boolean(!has_it)));
            }
            return Some(Ok(JsValue::Boolean(false)));
        }
        if let Some(arg) = class_rest.strip_prefix("contains(").and_then(|s| s.strip_suffix(")")) {
            let cls = unquote(arg);
            let has_it = tree.get_node(id)
    ...
    if let Some(args) = rest.strip_prefix(".replaceChild(").and_then(|s| s.strip_suffix(")")) {
        if let Some((new_arg, old_arg)) = args.split_once(',') {
            if let (Some(new_id), Some(old_id)) = (parse_node_id_arg(new_arg.trim()), parse_node_id_arg(old_arg.trim())) {
...
fn parse_node_id_arg(s: &str) -> Option<usize> {
    // Direct numeric id
    if let Ok(id) = s.parse::<usize>() { return Some(id); }
...
fn find_by_id(tree: &DomTree, id: &str) -> Option<usize> {
    tree.nodes
        .iter()
        .find(|n| n.node_type == NodeType::Element && n.attributes.get("id").map(|s| s.as_str()) == Some(id))
        .map(|n| n.id)
}
...
mod tests {
    ...
    fn set_and_get_attribute() {
        ...
        eval_dom(&mut t, "document.getElementById('n').setAttribute('value','42')").unwrap().unwrap();
        let got = eval_dom(&mut t, "document.getElementById('n').getAttribute('value')").unwrap().unwrap();
    ...
    fn query_selector_by_class_and_tag() {
        ...
        let by_class = eval_dom(&mut t, "document.querySelector('.lead').textContent").unwrap().unwrap();
        assert_eq!(by_class, JsValue::String("Intro".to_string()));
        let by_tag = eval_dom(&mut t, "document.querySelector('p').tagName").unwrap().unwrap();
    ...
    fn missing_element_is_null() {
        ...
        let res = eval_dom(&mut t, "document.getElementById('nope').textContent").unwrap().unwrap();
