...
impl JsVirtualMachine {
    pub fn new() -> Self {
        ...
        // Register built-in globals in the scope chain
        {
            ...
            s.locals.insert(
                "parseInt".to_string(),
                JsValue::NativeFunction("parseInt".into()),
            );
            s.locals.insert(
                "parseFloat".to_string(),
                JsValue::NativeFunction("parseFloat".into()),
            );
            s.locals
                .insert("isNaN".to_string(), JsValue::NativeFunction("isNaN".into()));
            s.locals.insert(
                "isFinite".to_string(),
                JsValue::NativeFunction("isFinite".into()),
            );
            s.locals
                .insert("eval".to_string(), JsValue::NativeFunction("eval".into()));
            s.locals.insert(
                "encodeURIComponent".to_string(),
                JsValue::NativeFunction("encodeURIComponent".into()),
            );
            s.locals.insert(
                "decodeURIComponent".to_string(),
                JsValue::NativeFunction("decodeURIComponent".into()),
            );
            ...
            math.insert(
                "floor".to_string(),
                JsValue::NativeFunction("Math.floor".into()),
            );
            math.insert(
                "ceil".to_string(),
                JsValue::NativeFunction("Math.ceil".into()),
            );
            math.insert(
                "round".to_string(),
                JsValue::NativeFunction("Math.round".into()),
            );
            math.insert(
                "abs".to_string(),
                JsValue::NativeFunction("Math.abs".into()),
            );
            math.insert(
                "max".to_string(),
                JsValue::NativeFunction("Math.max".into()),
            );
            math.insert(
                "min".to_string(),
                JsValue::NativeFunction("Math.min".into()),
            );
            math.insert(
                "random".to_string(),
                JsValue::NativeFunction("Math.random".into()),
            );
            math.insert(
                "sqrt".to_string(),
                JsValue::NativeFunction("Math.sqrt".into()),
            );
            math.insert(
                "pow".to_string(),
                JsValue::NativeFunction("Math.pow".into()),
            );
            math.insert(
                "log".to_string(),
                JsValue::NativeFunction("Math.log".into()),
            );
            math.insert(
                "trunc".to_string(),
                JsValue::NativeFunction("Math.trunc".into()),
            );
            math.insert(
                "sign".to_string(),
                JsValue::NativeFunction("Math.sign".into()),
            );
            ...
            json_obj.insert(
                "parse".to_string(),
                JsValue::NativeFunction("JSON.parse".into()),
            );
            json_obj.insert(
                "stringify".to_string(),
                JsValue::NativeFunction("JSON.stringify".into()),
            );
            s.locals
                .insert("JSON".to_string(), JsValue::Object(json_obj));

            // Object constructor
            let mut obj_ctor = HashMap::new();
            obj_ctor.insert(
                "keys".to_string(),
                JsValue::NativeFunction("Object.keys".into()),
            );
            obj_ctor.insert(
                "values".to_string(),
                JsValue::NativeFunction("Object.values".into()),
            );
            obj_ctor.insert(
                "assign".to_string(),
                JsValue::NativeFunction("Object.assign".into()),
            );
            obj_ctor.insert(
                "entries".to_string(),
                JsValue::NativeFunction("Object.entries".into()),
            );
            obj_ctor.insert(
                "freeze".to_string(),
                JsValue::NativeFunction("Object.freeze".into()),
            );
            s.locals
                .insert("Object".to_string(), JsValue::Object(obj_ctor));

            // Array constructor
            let mut arr_ctor = HashMap::new();
            arr_ctor.insert(
                "isArray".to_string(),
                JsValue::NativeFunction("Array.isArray".into()),
            );
            arr_ctor.insert(
                "from".to_string(),
                JsValue::NativeFunction("Array.from".into()),
            );
            s.locals
                .insert("Array".to_string(), JsValue::Object(arr_ctor));
            ...
            s.locals
                .insert("Number".to_string(), JsValue::Object(num_ctor));

            // String constructor
            let mut str_ctor = HashMap::new();
            str_ctor.insert(
                "fromCharCode".to_string(),
                JsValue::NativeFunction("String.fromCharCode".into()),
            );
            s.locals
                .insert("String".to_string(), JsValue::Object(str_ctor));

            // Date
            let mut date_ctor = HashMap::new();
            date_ctor.insert(
                "now".to_string(),
                JsValue::NativeFunction("Date.now".into()),
            );
            s.locals
                .insert("Date".to_string(), JsValue::Object(date_ctor));

            // console
            let mut console = HashMap::new();
            console.insert(
                "log".to_string(),
                JsValue::NativeFunction("console.log".into()),
            );
            console.insert(
                "warn".to_string(),
                JsValue::NativeFunction("console.warn".into()),
            );
            console.insert(
                "error".to_string(),
                JsValue::NativeFunction("console.error".into()),
            );
            console.insert(
                "info".to_string(),
                JsValue::NativeFunction("console.info".into()),
            );
            s.locals
                .insert("console".to_string(), JsValue::Object(console));

            // window and document will be populated per-session
            s.locals
                .insert("window".to_string(), JsValue::Object(HashMap::new()));
            s.locals
                .insert("document".to_string(), JsValue::Object(HashMap::new()));

            // Symbol constructor + well-known symbols
            let mut symbol_obj = HashMap::new();
            symbol_obj.insert(
                "iterator".to_string(),
                JsValue::String("__symbol_iterator__".into()),
            );
            symbol_obj.insert(
                "toPrimitive".to_string(),
                JsValue::String("__symbol_toPrimitive__".into()),
            );
            symbol_obj.insert(
                "hasInstance".to_string(),
                JsValue::String("__symbol_hasInstance__".into()),
            );
            symbol_obj.insert(
                "for".to_string(),
                JsValue::NativeFunction("Symbol.for".into()),
            );
            s.locals
                .insert("Symbol".to_string(), JsValue::Object(symbol_obj));

            // globalThis = window
            s.locals
                .insert("globalThis".to_string(), JsValue::Object(HashMap::new()));
            s.locals
                .insert("self".to_string(), JsValue::Object(HashMap::new()));

            // structuredClone
            s.locals.insert(
                "structuredClone".to_string(),
                JsValue::NativeFunction("structuredClone".into()),
            );
            s.locals.insert(
                "queueMicrotask".to_string(),
                JsValue::NativeFunction("queueMicrotask".into()),
            );
            s.locals.insert(
                "requestAnimationFrame".to_string(),
                JsValue::NativeFunction("requestAnimationFrame".into()),
            );
            s.locals.insert(
                "requestIdleCallback".to_string(),
                JsValue::NativeFunction("requestIdleCallback".into()),
            );
            ...
            navigator.insert(
                "userAgent".to_string(),
                JsValue::String("Mozilla/5.0 (compatible; VelocityBrowser/1.0; AgentFirst)".into()),
            );
            ...
            s.locals
                .insert("navigator".to_string(), JsValue::Object(navigator));

            // Reflect (methods dispatched via call_native)
            let mut reflect = HashMap::new();
            reflect.insert(
                "get".to_string(),
                JsValue::NativeFunction("Reflect.get".into()),
            );
            reflect.insert(
                "set".to_string(),
                JsValue::NativeFunction("Reflect.set".into()),
            );
            reflect.insert(
                "has".to_string(),
                JsValue::NativeFunction("Reflect.has".into()),
            );
            reflect.insert(
                "deleteProperty".to_string(),
                JsValue::NativeFunction("Reflect.deleteProperty".into()),
            );
            reflect.insert(
                "ownKeys".to_string(),
                JsValue::NativeFunction("Reflect.ownKeys".into()),
            );
            reflect.insert(
                "getOwnPropertyDescriptor".to_string(),
                JsValue::NativeFunction("Reflect.getOwnPropertyDescriptor".into()),
            );
            reflect.insert(
                "apply".to_string(),
                JsValue::NativeFunction("Reflect.apply".into()),
            );
            reflect.insert(
                "construct".to_string(),
                JsValue::NativeFunction("Reflect.construct".into()),
            );
            s.locals
                .insert("Reflect".to_string(), JsValue::Object(reflect));
            ...
            history.insert(
                "pushState".to_string(),
                JsValue::NativeFunction("__noop__".into()),
            );
            history.insert(
            ...
            s.locals
                .insert("history".to_string(), JsValue::Object(history));
            ...
            s.locals
                .insert("location".to_string(), JsValue::Object(location));
            ...
            s.locals
                .insert("crypto".to_string(), JsValue::Object(crypto));
            ...
            s.locals
                .insert("performance".to_string(), JsValue::Object(perf));
        }
    ...
    pub fn dispatch_event(
        ...
        let node_id =
            crate::js::dom_api::eval_dom(tree, &format!("document.querySelector('{}')", selector))
                .and_then(|r| r.ok())
                .and_then(|v| {
                    if let JsValue::Object(m) = v {
                        m.get("__node_id__").and_then(|n| {
                            if let JsValue::Number(id) = n {
                                Some(*id as usize)
                            } else {
                                None
                            }
        ...
        if let Some(nid) = node_id {
            ...
            for listener in matching {
                ...
                let _ = crate::js::interpreter::call_function(
                    &listener.handler,
                    &[JsValue::Object(event_obj)],
                    &self.scope_ref,
                );
                triggered += 1;
            }
        ...
        Ok(format!(
            "Dispatched {} event to '{}' (triggered {} listeners)",
            event, selector, triggered
        ))
    }
    ...
    pub fn remove_fn_listener(&mut self, node_id: usize, event_type: &str) {
        self.fn_listeners
            .retain(|l| !(l.node_id == node_id && l.event_type == event_type));
    }
    ...
    pub fn dispatch_event_by_node(&mut self, node_id: usize, event: &str) -> Vec<JsValue> {
        ...
        for listener in matching {
            ...
            if let Ok(v) = crate::js::interpreter::call_function(
                &listener.handler,
                &[JsValue::Object(event_obj)],
                &self.scope_ref,
            ) {
...
mod tests {
    ...
    fn evaluates_multiple_statements_returns_last() {
        ...
        let out = vm
            .eval_statement(&mut tree, "var a = 2; var b = 3; a + b")
            .unwrap();
    ...
    fn bare_assignment_updates_scope() {
        let mut vm = JsVirtualMachine::new();
        let mut tree = empty_tree();
        vm.eval_statement(&mut tree, "let x = 10; x = x + 5")
            .unwrap();
    ...
    fn dom_mutation_flows_through_statement() {
        ...
        vm.eval_statement(
            &mut tree,
            "document.getElementById('f').setAttribute('value','hi')",
        )
        .unwrap();
        let node = tree
            .nodes
            .iter()
            .find(|n| n.attributes.get("id").map(|s| s.as_str()) == Some("f"))
            .unwrap();
    ...
    fn if_else_control_flow() {
        ...
        let mut tree = empty_tree();
        let out = vm
            .eval_statement(&mut tree, "var x = 10; if (x > 5) { x = x * 2; } x")
            .unwrap();
