...
Both the MCP tool and the agent-to-UI channel share a single serialisation path via `pub fn fetch_panel_data_value()` in `system_tools.rs`, which returns a `serde_json::Value`.
