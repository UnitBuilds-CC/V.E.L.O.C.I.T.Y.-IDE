...
**VC** in V.E.L.O.C.I.T.Y. stands for **Version Control**, specifically referring to our custom, zero-external-dependency **Merkle AST Version Control System** implemented in `velocity-ide/src/site_map/`.
...
- **`MerkleVerifier`**: Computes deterministic root hashes (`root` and `weight_root`) to verify code integrity and detect corruption without parsing raw files.

### 3. Workspace File Tree & Symbol History Explorer
Integrated into `velocity-mcp/src/editor/graph_view.rs`:
...
2. **Zero External Dependency**: Runs natively in Rust inside `velocity-ide` and `velocity-mcp`.
