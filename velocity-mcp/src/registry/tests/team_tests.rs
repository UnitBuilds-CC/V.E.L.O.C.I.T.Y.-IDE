...
fn preset_templates_load_correctly() {
    ...
    assert!(template_names.contains(&"Graphic Design & Branding Team"));
    assert!(template_names.contains(&"Healthcare & Medical Research Team"));
    assert!(template_names.contains(&"Aerospace Engineering Team"));
    assert!(template_names.contains(&"Energy & Utilities Management Team"));
}
