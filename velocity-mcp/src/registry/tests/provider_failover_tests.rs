...
fn member_with_fallback_includes_field() {
    ...
    assert!(json.contains("fallback_provider"));
    assert!(
        json.contains("CloudflareWorkersAi"),
        "expected CloudflareWorkersAi in: {}",
        json
    );
}
...
fn effective_provider_uses_member_when_model_set() {
    ...
    let (provider, model) =
        member.resolve_effective_provider_and_model(AiProvider::OpenAI, "gpt-4-default");
...
fn effective_provider_falls_back_when_model_whitespace() {
    ...
    let (provider, model) =
        member.resolve_effective_provider_and_model(AiProvider::Groq, "llama-3");
...
fn route_narrow_scope_beats_broad_scope() {
    ...
    let routed = route_member(&team, "anything", &["src/net/tcp.rs".to_string()], None).unwrap();
...
fn route_keyword_match_beats_fallback() {
    let team = sample_team();
    let routed = route_member(&team, "build the database layer", &[], None).unwrap();
...
fn route_no_match_falls_back_to_lead() {
    let team = sample_team();
    let routed = route_member(&team, "completely unrelated task xyz", &[], None).unwrap();
...
fn route_with_custom_router() {
    ...
    let routed = route_member(&team, "ambiguous task", &[], Some(&router)).unwrap();
    assert_eq!(routed.member_id, "backend");
...
fn debug_routing_shows_file_scope_stage() {
    let team = sample_team();
    let decision = debug_routing(&team, "do something", &["src/ui/button.rs".to_string()]);
...
fn debug_routing_shows_keyword_stage() {
    let team = sample_team();
    let decision = debug_routing(&team, "build the database schema", &[]);
...
fn debug_routing_shows_fallback_stage() {
    let team = sample_team();
    let decision = debug_routing(&team, "xyzzy nonsense task", &[]);
...
fn team_with_fallback_roundtrips_through_save_load() {
    ...
    assert_eq!(
        loaded[0].members[0].fallback_provider,
        Some(AiProvider::Anthropic)
    );
    assert_eq!(
        loaded[0].members[1].fallback_provider,
