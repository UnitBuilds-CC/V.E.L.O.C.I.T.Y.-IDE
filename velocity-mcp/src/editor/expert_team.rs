...
pub struct ExpertMember {
    /// Internal id; regenerated by `import_team_from_json` when omitted.
    #[serde(default)]
    pub id: String,
    #[serde(default)]
    pub name: String,
    #[serde(default)]
    pub role: String,
    /// Lenient: accepts variant name, display label, or slug (see the
    /// `Deserialize` impl on `AiProvider`). Missing → CloudflareWorkersAi,
    /// mirroring `parse_member` so hand-written team JSON imports the same
    /// way tool-created members do.
    #[serde(default = "default_member_provider")]
...
fn default_member_provider() -> AiProvider {
...
pub struct ExpertTeam {
    /// Internal id; regenerated from the name by `import_team_from_json`,
    /// so hand-written team JSON does not need it.
    #[serde(default)]
    pub id: String,
    pub name: String,
    #[serde(default)]
...
impl ExpertTeam {
    ...
    fn find_member_index(&self, needle: &str) -> Option<usize> {
        ...
        if let Some(i) = self
            .members
            .iter()
            .position(|m| m.name.to_lowercase() == lower)
        {
            return Some(i);
        }
        if let Some(i) = self.members.iter().position(|m| slugify(&m.name) == lower) {
            return Some(i);
        }
        None
    }
