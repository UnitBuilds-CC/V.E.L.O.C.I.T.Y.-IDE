...
impl VelocityApp {
    ...
    pub fn mru_overlay_ui(&mut self, ctx: &egui::Context) {
        ...
        if !self.mru.open {
            if tab_pressed {
                let dock_tabs: Vec<Tab> = self.dock_state.as_ref()
                    .map(|d| d.iter_all_tabs().map(|(_, t)| t.clone()).collect()).unwrap_or_default();
                if dock_tabs.len() >= 2 {
                    let mut order: Vec<TabId> = Vec::new();
                    if let Some(active) = self.active_tab.as_ref() { order.push(active.clone()); }
                    for t in &dock_tabs { if !order.contains(&t.id) { order.push(t.id.clone()); } }
                    self.mru.order = order;
                    self.mru.selected = 1.min(self.mru.order.len().saturating_sub(1));
                    self.mru.open = true;
                }
            }
            if !self.mru.open { return; }
        }
        if !cmd_held {
            let chosen = self.mru.order.get(self.mru.selected).cloned();
            self.mru.open = false;
            if let Some(id) = chosen { self.activate_tab_by_id(&id); }
            return;
        }
        if tab_pressed {
            let len = self.mru.order.len();
            if len > 0 {
                if shift { self.mru.selected = self.mru.selected.checked_sub(1).unwrap_or(len - 1); }
                else { self.mru.selected = (self.mru.selected + 1) % len; }
            }
        }
        let palette = self.palette();
        ...
        egui::Area::new(egui::Id::new("mru_overlay_area"))
            ...
            .show(ctx, |ui| {
                egui::Frame::popup(ui.style()).fill(ui.visuals().code_bg_color)
                    .stroke(ui.visuals().window_stroke).inner_margin(egui::Margin::same(10))
                    .corner_radius(egui::CornerRadius::same(12)).show(ui, |ui| {
                        ui.set_min_width(300.0);
                        ui.label(egui::RichText::new("Switch Tab").size(12.0).color(palette.text_muted));
                        ui.add_space(4.0);
                        egui::ScrollArea::vertical().max_height(320.0).show(ui, |ui| {
                            for (idx, id) in order.iter().enumerate() {
                                let title = self.tabs.iter().find(|t| &t.id == id)
                                    .map(|t| t.title()).unwrap_or_else(|| "(closed)".to_string());
                                let is_sel = idx == selected;
                                let resp = ui.selectable_label(is_sel,
                                    egui::RichText::new(title).color(if is_sel { palette.accent } else { palette.text }));
                                if is_sel { resp.scroll_to_me(Some(egui::Align::Center)); }
                                if resp.clicked() {
                                    let chosen = id.clone();
                                    self.mru.open = false;
    ...
    pub fn file_dialog_ui(&mut self, ctx: &egui::Context) {
        ...
        if !open { return; }
        let mut path_string = self.pending_open_path.as_ref()
            .and_then(|p| p.to_str()).map(String::from).unwrap_or_default();
        let palette = self.palette();
        let workspace_root = self.workspace_root.clone();
        egui::Window::new("Open File").open(&mut open).resizable(true)
            .collapsible(false).default_size((480.0, 360.0))
                    ...
                    ui.vertical(|ui| {
                        ...
                        ui.horizontal(|ui| {
                            if ui.button("Open").clicked() {
                                ...
                                if p.exists() && p.is_file() { self.open_editor(Some(p)); self.pending_open_path = None; }
                                else { self.status_message = format!("File not found: {}", p.display()); }
                            }
                            if ui.button("Cancel").clicked() { self.pending_open_path = None; }
                        });
                    });
                });
            });
        if !open { self.pending_open_path = None; }
    }

    fn render_file_tree_node(ui: &mut egui::Ui, node: &FileNode, workspace_root: &Path,
        path_string: &mut String, palette: crate::editor::theme::IdePalette,
    ) {
        if node.is_dir {
            if let Some(children) = &node.children {
                ...
                ).default_open(node.path == workspace_root).show(ui, |ui| {
            ...
            let rel = node.path.strip_prefix(workspace_root).unwrap_or(&node.path).to_string_lossy().to_string();
            let icon = crate::editor::search::icon_for_path(&node.path);
            if ui.add(egui::Button::new(
                egui::RichText::new(format!("{} {}", icon, rel)).size(9.0).color(palette.text)
            ).frame(false)).clicked() { *path_string = rel; }
        }
    }
    ...
    pub fn save_as_dialog_ui(&mut self, ctx: &egui::Context) {
        ...
        if !open { return; }
        let mut path_string = self.pending_save_as_path.as_ref()
            .and_then(|p| p.to_str()).map(String::from).unwrap_or_default();
        egui::Window::new("Save As").open(&mut open).resizable(false)
            .collapsible(false).anchor(egui::Align2::CENTER_CENTER, egui::Vec2::ZERO).show(ctx, |ui| {
                    ...
                    if ui.button("Save").clicked() {
                        if let Some(id) = self.active_tab.clone() {
                            ...
                            if let Some(tab) = self.tabs.iter_mut().find(|t| t.id == id) {
                                if let TabKind::Editor { ref mut path, .. } = tab.kind { *path = Some(p); }
                            }
                            self.pending_save_as_path = None;
                        }
                    }
                    if ui.button("Cancel").clicked() { self.pending_save_as_path = None; }
                });
            });
        if !open { self.pending_save_as_path = None; }
    }
    ...
    pub fn confirm_close_dialog_ui(&mut self, ctx: &egui::Context) {
        let Some(id) = self.pending_close_tab.clone() else { return; };
        if !self.tab_is_dirty(&id) {
            self.pending_close_tab = None; self.close_tab(&id); self.rebuild_dock(); return;
        }
        let palette = self.palette();
        let name = self.tab_path(&id).and_then(|p| p.file_name())
            .map(|n| n.to_string_lossy().to_string()).unwrap_or_else(|| "untitled".to_string());
        let mut resolved: Option<&'static str> = None;
        egui::Window::new("Unsaved changes").resizable(false).collapsible(false)
            .anchor(egui::Align2::CENTER_CENTER, egui::Vec2::ZERO).show(ctx, |ui| {
                ui.label(egui::RichText::new(format!("“{name}” has unsaved changes.")).color(palette.text));
                ui.label(egui::RichText::new("Do you want to save before closing?").small().color(palette.text_muted));
                ui.add_space(8.0);
                ui.horizontal(|ui| {
                    if ui.button(egui::RichText::new("Save").color(palette.success)).clicked() { resolved = Some("save"); }
                    if ui.button(egui::RichText::new("Don't Save").color(palette.warning)).clicked() { resolved = Some("discard"); }
                    if ui.button("Cancel").clicked() { resolved = Some("cancel"); }
                });
            });
        match resolved {
            Some("save") => {
                if let Some(path) = self.tab_path(&id).cloned() {
                    if self.save_buffer_to(&id, &path) {
                        self.pending_close_tab = None; self.close_tab(&id); self.rebuild_dock();
                    }
                } else { self.active_tab = Some(id); self.pending_close_tab = None; self.save_active_as(); }
            }
            Some("discard") => {
                if let Some(buf) = self.buffers.get_mut(&id) { buf.mark_saved(); }
                self.pending_close_tab = None; self.close_tab(&id); self.rebuild_dock();
            }
            Some("cancel") => { self.pending_close_tab = None; }
            _ => {}
        }
    }

    pub fn full_diff_ui(&mut self, ctx: &egui::Context) {
        if !self.show_full_diff { return; }
        let palette = self.palette();
        let mut open = self.show_full_diff;
        let active_change_preview = self.active_change_preview();
        egui::Window::new("Full Diff").open(&mut open).resizable(true)
            .default_size(egui::vec2(720.0, 520.0)).show(ctx, |ui| {
                if let Some(cp) = &active_change_preview {
                    ui.label(egui::RichText::new(format!("{}  (+{} / -{})",
                        cp.file_label, cp.added_lines, cp.removed_lines)).strong().color(palette.warning));
                    ui.add_space(6.0);
                    egui::ScrollArea::vertical().show(ui, |ui| {
                        ui.label(egui::RichText::new(cp.full_diff.as_str()).monospace().size(10.0).color(palette.text));
                    });
                } else { ui.label("No active unsaved changes."); }
            });
