...
impl VulkanDriver {
    ...
    pub fn run_attn_benchmarks(&self) -> Result<(f64, f64), Box<dyn std::error::Error>> {
        ...
        let create_coherent_buffer = |size: vk::DeviceSize,
        ...
        > {
            ...
            let memory_type_index =
                mem_type_index.ok_or("No host-visible coherent memory found.")?;
            let alloc_info = vk::MemoryAllocateInfo::builder()
                .allocation_size(mem_reqs.size)
                .memory_type_index(memory_type_index);
        ...
        let (q_buffer, q_memory, q_ptr) =
            create_coherent_buffer(q_size, vk::BufferUsageFlags::STORAGE_BUFFER)?;
        let (k_buffer, k_memory, k_ptr) =
            create_coherent_buffer(k_size, vk::BufferUsageFlags::STORAGE_BUFFER)?;
        let (v_buffer, v_memory, v_ptr) =
            create_coherent_buffer(v_size, vk::BufferUsageFlags::STORAGE_BUFFER)?;
        let (block_buffer, block_memory, block_ptr) =
            create_coherent_buffer(block_size, vk::BufferUsageFlags::STORAGE_BUFFER)?;
        let (out_buffer, out_memory, _out_ptr) =
            create_coherent_buffer(out_size, vk::BufferUsageFlags::STORAGE_BUFFER)?;
...
impl VulkanQwenLayer {
    pub fn new(
        ...
        unsafe {
            ...
            let dispatch_gemv = |cmd: vk::CommandBuffer,
                                 pipe: vk::Pipeline,
...
impl Drop for VulkanQwenLayer {
    fn drop(&mut self) {
        unsafe {
            ...
            destroy_buffer(
                &self.device,
                self.inputs_2304_buffer,
                self.inputs_2304_memory,
                true,
            );
            destroy_buffer(
                &self.device,
                self.out_2304_a_buffer,
                self.out_2304_a_memory,
                true,
            );
            destroy_buffer(
                &self.device,
                self.out_2304_b_buffer,
                self.out_2304_b_memory,
                false,
            );
            destroy_buffer(
                &self.device,
                self.out_256_k_buffer,
                self.out_256_k_memory,
                false,
            );
            destroy_buffer(
                &self.device,
                self.out_256_v_buffer,
                self.out_256_v_memory,
                false,
            );
            destroy_buffer(
                &self.device,
                self.out_11008_gate_buffer,
                self.out_11008_gate_memory,
                false,
            );
            destroy_buffer(
                &self.device,
                self.out_11008_up_buffer,
                self.out_11008_up_memory,
                false,
            );
            destroy_buffer(
                &self.device,
                self.inputs_11008_buffer,
                self.inputs_11008_memory,
                false,
            );

            destroy_buffer(
                &self.device,
                self.weight_q_buffer,
                self.weight_q_memory,
                false,
            );
            destroy_buffer(
                &self.device,
                self.weight_k_buffer,
                self.weight_k_memory,
                false,
            );
            destroy_buffer(
                &self.device,
                self.weight_v_buffer,
                self.weight_v_memory,
                false,
            );
            destroy_buffer(
                &self.device,
                self.weight_o_buffer,
                self.weight_o_memory,
                false,
            );
            destroy_buffer(
                &self.device,
                self.weight_gate_buffer,
                self.weight_gate_memory,
                false,
            );
            destroy_buffer(
                &self.device,
                self.weight_up_buffer,
                self.weight_up_memory,
                false,
            );
            destroy_buffer(
                &self.device,
                self.weight_down_buffer,
                self.weight_down_memory,
                false,
            );
...
impl VulkanBitNetLayer {
    pub fn new(
        ...
        let (weight_q_buffer, weight_q_memory) = create_device_local_buffer(
            ...
            queue,
            driver.queue_family_index,
            weight_q_packed.len() as vk::DeviceSize,
            &weight_q_packed,
        )?;
        let (weight_k_buffer, weight_k_memory) = create_device_local_buffer(
            &device,
            ...
            queue,
            driver.queue_family_index,
            weight_k_packed.len() as vk::DeviceSize,
            &weight_k_packed,
        )?;
        let (weight_v_buffer, weight_v_memory) = create_device_local_buffer(
            &device,
            ...
            queue,
            driver.queue_family_index,
            weight_v_packed.len() as vk::DeviceSize,
            &weight_v_packed,
        )?;
        let (weight_o_buffer, weight_o_memory) = create_device_local_buffer(
            &device,
            ...
            queue,
            driver.queue_family_index,
            weight_o_packed.len() as vk::DeviceSize,
            &weight_o_packed,
        )?;
        let (weight_gate_buffer, weight_gate_memory) = create_device_local_buffer(
            &device,
            ...
            queue,
            driver.queue_family_index,
            weight_gate_packed.len() as vk::DeviceSize,
            &weight_gate_packed,
        )?;
        let (weight_up_buffer, weight_up_memory) = create_device_local_buffer(
            &device,
            ...
            queue,
            driver.queue_family_index,
            weight_up_packed.len() as vk::DeviceSize,
            &weight_up_packed,
        )?;
        let (weight_down_buffer, weight_down_memory) = create_device_local_buffer(
            &device,
            ...
            queue,
            driver.queue_family_index,
            weight_down_packed.len() as vk::DeviceSize,
            &weight_down_packed,
        )?;
...
impl Drop for VulkanBitNetLayer {
    fn drop(&mut self) {
        unsafe {
            ...
            destroy_buffer(
                &self.device,
                self.inputs_3200_buffer,
                self.inputs_3200_memory,
                true,
            );
            destroy_buffer(
                &self.device,
                self.out_3200_down_buffer,
                self.out_3200_down_memory,
                true,
            );
            destroy_buffer(
                &self.device,
                self.out_3200_q_buffer,
                self.out_3200_q_memory,
                false,
            );
            destroy_buffer(
                &self.device,
                self.out_3200_k_buffer,
                self.out_3200_k_memory,
                false,
            );
            destroy_buffer(
                &self.device,
                self.out_3200_v_buffer,
                self.out_3200_v_memory,
                false,
            );
            destroy_buffer(
                &self.device,
                self.out_3200_o_buffer,
                self.out_3200_o_memory,
                false,
            );
            destroy_buffer(
                &self.device,
                self.out_8640_gate_buffer,
                self.out_8640_gate_memory,
                false,
            );
            destroy_buffer(
                &self.device,
                self.out_8640_up_buffer,
                self.out_8640_up_memory,
                false,
            );
            destroy_buffer(
                &self.device,
                self.inputs_8640_buffer,
                self.inputs_8640_memory,
                false,
            );
...
impl VulkanNdaBitNetLayer {
    pub fn new(
        ...
        // 1. Create Shaders
        let shader_nda = unsafe {
            device.create_shader_module(
                &vk::ShaderModuleCreateInfo::builder().code(crate::compiler::shaders::NDA_SPV),
                None,
            )?
        };
        let shader_act = unsafe {
            device.create_shader_module(
        ...
        // Pack weights
        let create_nda_weight_buffers = |device: &Device,
                                         src: &[u8],
                                         k: usize,
                                         n: usize|
         -> Result<
        ...
        > {
            ...
            let (ab, am) = create_device_local_buffer(
                device,
                &instance,
                physical_device,
                queue,
                driver.queue_family_index,
                act_bytes.len() as vk::DeviceSize,
                &act_bytes,
            )?;
            let (pb, pm) = create_device_local_buffer(
                device,
                &instance,
                physical_device,
                queue,
                driver.queue_family_index,
                pos_bytes.len() as vk::DeviceSize,
                &pos_bytes,
            )?;
            Ok((ab, am, pb, pm))
        };
        ...
        unsafe {
            ...
            let dispatch_nda = |cmd: vk::CommandBuffer,
                                pipe: vk::Pipeline,
                                layout: vk::PipelineLayout,
                                set: vk::DescriptorSet,
                                k: u32,
...
impl Drop for VulkanNdaBitNetLayer {
    fn drop(&mut self) {
        unsafe {
            ...
            destroy_buffer(
                &self.device,
                self.inputs_3200_active_buffer,
                self.inputs_3200_active_memory,
                true,
            );
            destroy_buffer(
                &self.device,
                self.inputs_3200_pos_buffer,
                self.inputs_3200_pos_memory,
                true,
            );
            destroy_buffer(
                &self.device,
            ...
            destroy_buffer(
                &self.device,
                self.inputs_8640_active_buffer,
                self.inputs_8640_active_memory,
                false,
            );
            destroy_buffer(
                &self.device,
                self.inputs_8640_pos_buffer,
                self.inputs_8640_pos_memory,
                false,
            );

            destroy_buffer(
                &self.device,
                self.weight_q_active_buffer,
                self.weight_q_active_memory,
                false,
            );
            destroy_buffer(
                &self.device,
                self.weight_q_pos_buffer,
                self.weight_q_pos_memory,
                false,
            );
            destroy_buffer(
                &self.device,
                self.weight_k_active_buffer,
                self.weight_k_active_memory,
                false,
            );
            destroy_buffer(
                &self.device,
                self.weight_k_pos_buffer,
                self.weight_k_pos_memory,
                false,
            );
            destroy_buffer(
                &self.device,
                self.weight_v_active_buffer,
                self.weight_v_active_memory,
                false,
            );
            destroy_buffer(
                &self.device,
                self.weight_v_pos_buffer,
                self.weight_v_pos_memory,
                false,
            );
            destroy_buffer(
                &self.device,
                self.weight_o_active_buffer,
                self.weight_o_active_memory,
                false,
            );
            destroy_buffer(
                &self.device,
                self.weight_o_pos_buffer,
                self.weight_o_pos_memory,
                false,
            );
            destroy_buffer(
                &self.device,
                self.weight_gate_active_buffer,
                self.weight_gate_active_memory,
                false,
            );
            destroy_buffer(
                &self.device,
                self.weight_gate_pos_buffer,
                self.weight_gate_pos_memory,
                false,
            );
            destroy_buffer(
                &self.device,
                self.weight_up_active_buffer,
                self.weight_up_active_memory,
                false,
            );
            destroy_buffer(
                &self.device,
                self.weight_up_pos_buffer,
                self.weight_up_pos_memory,
                false,
            );
            destroy_buffer(
                &self.device,
                self.weight_down_active_buffer,
                self.weight_down_active_memory,
                false,
            );
            destroy_buffer(
                &self.device,
                self.weight_down_pos_buffer,
                self.weight_down_pos_memory,
                false,
            );
